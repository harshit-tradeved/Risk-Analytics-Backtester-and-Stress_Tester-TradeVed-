...
from backtesting.backend.data.fetcher import DataFetcher, is_forex_pair
from backtesting.backend.data.validator import DataValidator
from backtesting.backend.engine.metrics import score_backtest, score_breakdown
from backtesting.backend.engine.validation import run_segment_backtest, run_holdout, run_walk_forward
...
def extract_ir(transcript: str, caption: str, tweak: Optional[str] = None) -> dict[str, Any]:
    ...
    from reel_to_backtest.backend.reel_extractor import extract_strategy_ir
...
def validate_and_repair(ir: dict[str, Any]) -> tuple[dict[str, Any], list[str]]:
    ...
    from reel_to_backtest.backend.improvement_agent import repair_improved_ir
...
def build_fallback_suggestion(transcript: str, caption: str, gaps: list[str]) -> dict[str, Any]:
    ...
    from reel_to_backtest.backend.reel_extractor import suggest_fallback_ir
...
def patch_ir_with_tweak(ir: dict[str, Any], tweak: str, symbol: str, interval: str) -> dict[str, Any]:
    ...
    """
    from reel_to_backtest.backend.improvement_agent import _llm, _parse_json  # same LLM plumbing critique_and_improve uses
