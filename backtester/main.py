...
from backtesting.backend.data.indian_assets import (
...
from backtesting.backend.engine.regimes import classify_regimes, regime_breakdown
from backtesting.backend.engine.validation import run_holdout, run_walk_forward
from stress_testing.backend.stress import (
...
from reel_to_pipeline.backend import pipeline as pipeline_orchestrator
...
def list_indicators():
    ...
    """
    from backtesting.backend.engine.indicators import INDICATOR_CATALOG, GROUPS, TOTAL_OUTPUT_SERIES
    return {
        "groups": list(GROUPS),
...
def backtest_from_ir(req: FromIRRequest, db: Session = Depends(get_db)):
    ...
    """
    from reel_to_backtest.backend.ir_validator import validate_ir, normalize_ir
...
async def analyze_reel(req: ReelAnalyzeRequest):
    ...
    """
    from reel_to_backtest.backend.reel_extractor import triage, extract_strategy_ir
    from reel_to_backtest.backend.ir_validator import validate_ir
    ...
    if not transcript:
        ...
        if INGESTION_API_URL:
            ...
            try:
                from reel_to_backtest.backend.ingestion import handle_url as _ingest_url
    ...
    from reel_to_backtest.backend.ir_validator import normalize_ir
...
def improve_reel_strategy(req: ImproveRequest, db: Session = Depends(get_db)):
    ...
    """
    from reel_to_backtest.backend.ir_validator import validate_ir, normalize_ir
    from reel_to_backtest.backend.improvement_agent import critique_and_improve, repair_improved_ir, compute_diff, judge_pipeline
...
def _prepare_forecast(req: ForecastRequest):
    ...
    from stress_testing.backend.stress import StressScenario
    if req.strategy.upper() not in STRATEGY_REGISTRY:
        raise HTTPException(400, f"Unknown strategy '{req.strategy}'")
...
def run_forecast(req: ForecastRequest):
    ...
    """
    from forward_testing.backend.forecast import generate_paths as _gen_paths
...
async def stream_forecast_sse(req: ForecastRequest):
    ...
    async def _generate():
        ...
        try:
            ...
            if _kurl:
                from forward_testing.backend.forecast import KronosClient as _KC
                _client = _KC(_kurl)
                _BATCH  = 10
                _specs  = [
                ...
                from forward_testing.backend.forecast import _block_bootstrap_paths as _bbp
...
async def compare_forecast_methods(req: ForecastRequest):
    ...
    """
    from forward_testing.backend.forecast import (
...
async def stream_paper_trade(req: PaperRequest):
    ...
    async def _gen_paper():
        ...
        try:
            from forward_testing.backend.forecast import generate_one_path as _gen_one_path
...
async def _refetch_pipeline_df(row: models.PipelineRun):
    from reel_to_pipeline.backend.stages import fetch_with_source_fallback
    from reel_to_pipeline.backend.pipeline import LOOKBACK_DAYS
...
async def pipeline_walk_forward(run_id: str, db: Session = Depends(get_db)):
    from reel_to_pipeline.backend.stages import run_walk_forward_detail
...
async def pipeline_stress_detail(run_id: str, db: Session = Depends(get_db)):
    from reel_to_pipeline.backend.stages import run_stress_detail
    row = _completed_pipeline_row(run_id, db)
    ir = json.loads(row.ir_json)
    df = await _refetch_pipeline_df(row)
    capital = row.capital or 10_000.0
