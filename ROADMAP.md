...
`backtester/backtesting/backend/engine/indicators.py` (pure pandas/numpy):
...
- `backtesting/backend/strategies/custom.py: CustomStrategy` — `entry_rules`/`exit_rules` as condition lists (`left_indicator, operator (>/</cross_above/cross_below), right: value|indicator`), combined via AND/OR; evaluator computes indicators via the engine and emits BUY/SELL/HOLD. Category `custom`.
...
- Signal→trade execution: `backtesting/backend/engine/simulator.py` (BUY/SELL/HOLD + `quantity`) — simulator, cost models, metrics, regimes, stress all work unchanged.
...
- [x] Track 1 / Phase A — indicator engine (`backtesting/backend/engine/indicators.py`, 25 indicators / 40 series) + `GET /api/indicators`
...
- [x] Track 1 / Phase D — rule builder (`backtesting/backend/strategies/custom.py` + `RuleBuilder.tsx`)
- [x] Track 2 / Phase K1 — AI Forward-Test (`forward_testing/backend/forecast.py` block-bootstrap + `POST /api/forecast/run|stream` + `ForwardTestPage` + `ForwardTestSidebar` + `ForwardTestResults`; Kronos GPU slots in via `KRONOS_URL` env var with zero code change)
